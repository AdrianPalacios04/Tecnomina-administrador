	...
	<div class="container">
		<div class="content">
			...
			<table class="table table-striped">
				...
				<?php
				//if($filter){
					//$sql = mysqli_query($con, "SELECT * FROM personal WHERE paterno='$filter'");
				//}else{
					$sql = mysqli_query($con, "SELECT * FROM personal ORDER BY dni ASC");
				//}
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						echo '
							...
							<td>
								...
								<a href="table.php?aksi=delete&nik='.$row['dni'].'" title="Eliminar" onclick="return confirm(\'Esta seguro de borrar los datos '.$row['nombre'].'?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
							</td>
						</tr>
						';
						$no++;
